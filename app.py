...
def format_time(
    time_value: Optional[Union[datetime, time, str, float]] = None,
    format_str: str = "%H:%M:%S",
    as_string: bool = True
) -> Union[str, tuple]:
    """
    Formatea un valor de tiempo en diferentes formatos.
    
    Args:
        time_value: Valor de tiempo a formatear. Puede ser:
                   - datetime: objeto datetime
                   - time: objeto time
                   - str: string de tiempo formateable
                   - float: timestamp
                   - None: hora actual
        format_str: Formato de salida (por defecto "%H:%M:%S")
        as_string: Si True retorna string, si False retorna tupla (horas, minutos, segundos)
    
    Returns:
        String formateado o tupla con componentes de tiempo
    
    Examples:
        >>> format_time()
        '14:30:25'
        >>> format_time("13:45:30", "%I:%M %p")
        '01:45 PM'
        >>> format_time(1672539032, "%H:%M")
        '14:30'
    """
    try:
        ...
        if time_value is None:
            ...
            # Intentar parsear string de tiempo
            try:
                ...
                try:
                    ...
                    raise ValueError("Formato de tiempo no reconocido. Use HH:MM:SS o HH:MM")
        elif isinstance(time_value, (int, float)):
            # Timestamp
            time_obj = datetime.fromtimestamp(time_value).time()
        else:
            raise ValueError("Tipo de tiempo no soportado")
        
        # Formatear salida
        if as_string:
            # Crear datetime ficticio para usar strftime
            dummy_datetime = datetime(2023, 1, 1, time_obj.hour, time_obj.minute, time_obj.second)
            return dummy_datetime.strftime(format_str)
        else:
            return (time_obj.hour, time_obj.minute, time_obj.second)
            
    except Exception as e:
        raise ValueError(f"Error formateando tiempo: {str(e)}")


# Función especializada para tiempos relativos
def format_duration(seconds: float, precise: bool = False) -> str:
    ...
    Args:
        ...
        precise: Si True muestra milisegundos
